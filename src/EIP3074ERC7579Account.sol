...
import { PackedUserOperation, IEntryPoint } from "modulekit/external/ERC4337.sol";
import { IERC7579Account, IERC7579Validator } from "modulekit/external/ERC7579.sol";
...
- merge with https://github.com/thogard785/generalized-interpretable-invoker/tree/main to serve the
same role
    - add staking support
    Optional
...
// @dev NOTE : this is vulnerable to DoS since actual validation for userOpHash is done on the
// execution side, figuring out the fixes though
contract EIP3074ERC7579Account is Auth, ExecutionHelper {
    ...
    mapping(address validatorModule => mapping(address account => ValidatorData data))
        $validatorData;
        ...
        returns (uint256 validationData)
    {
        ...
        address signer = ecrecover(
            getDigest(commit, nonce),
            uint8(bytes1(authSig[64])),
    ...
    function executeUserOp(PackedUserOperation calldata userOp, bytes32 userOpHash) external {
        ...
        if (accountSig == IERC7579Account.execute.selector) {
            // (CallType _calltype, ExecType _execType, ModeSelector _modeSelector, ModePayload
            // _modePayload) =
            //     ModeLib.decode();
        ...
        returns (uint256)
    {
        (bool success, bytes memory result) = authcall(
            validator,
            abi.encodeWithSelector(IERC7579Validator.validateUserOp.selector, op, userOpHash),
            0,
            gasleft()
        );
